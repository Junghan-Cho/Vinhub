...
- **프레임워크**: Next.js 16 (App Router, TypeScript).
  ...
  - 기존 `archive/vite-spa/src/pages/**`, `archive/vite-spa/src/App.tsx` 등은 과거 Vite/SPA 레거시로, **Next 빌드에서는 제외** (`tsconfig.json` exclude: `archive/**`). 새 기능은 `app/` 에만 추가.
- **언어/도구**:
  - TypeScript
...
- `data/` (및 `archive/vite-spa/src/data/` 참조)
  - **정식 데이터 소스는 `data/*`** 이다. (App Router 및 `lib/search/*` 는 `@/data/*`만 사용)
  - `archive/vite-spa/src/data/*`는 레거시(참고/이전용) 스냅샷이며, 신규 기능/수정은 `data/*`에 반영한다.
  - 주요 파일:
